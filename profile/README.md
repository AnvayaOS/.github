...
> **Now:** v1.7.2 closes audit findings F01–F19 across capability authority, IPC, driver and storage recovery, and native intelligence. Signed Broker, model and checkpoint-storage services run in separate U-mode tasks; the six-boot/four-crash-cut gate covers retained charges, replay refusal, restart liveness, and rollback/corruption/truncation denials. Protected freshness anchors and a durable clock floor survive recovery. The superseded privileged intelligence path is removed. Complete local Kani refinement, Verus/Lean, QEMU, native recovery, ACVP and documentation gates are pinned to the released source. The runtime keeps transport=service-loopback, polling=0 and kernel_fallbacks=0. This is QEMU-proven research software; M1, M6 and M8 remain PARTIAL, without physical-hardware or external-audit claims.
