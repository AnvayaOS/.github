...
> **Now:** v1.7.1 "Closed-scope repairs" closes the 20 findings in the already-delivered capability, driver, storage, network and verification paths. Capability ancestry and selective revocation survive dropped handles; signed Driver rights, DMA quotas and resident-page limits are enforced; exact capability sockets retain port/DNS restrictions and generation-safe identities. ABI 0.3 provides kernel-issued reply tickets and real U-mode cancellation receipts. Durable receive queues drain, a real 160-byte namespace CRDT delta merges in U-mode, storage headers protect commit metadata, and full-sector corruption is rejected before writing. CRDT failures are atomic, virtio indices wrap safely, block errors propagate, and future-dated energy inputs are refused. The runtime keeps transport=service-loopback, polling=0 and kernel_fallbacks=0. This remains a QEMU-proven research release; M1, M6 and M8 remain PARTIAL, with no physical-hardware or external-audit claim. Product core is 21,530 lines; nucleus/src is 22,259; core plus the 1,641-line transport extension is 23,171. The model disclosure counts 3,620 non-test source lines separately. The 25,605 combined release ceiling and 15,000 core target are unchanged.
